...
def load_train_data(dataset='dijon'):
    ...
    train_feature = train_feature.reshape(-1, num_band, time_step)
    # append vv-vh feature
    diff_feature = train_feature[:, 0, :] - train_feature[:, 1, :]
    diff_feature = diff_feature[:, np.newaxis, :]
    train_feature = np.concatenate((train_feature, diff_feature), axis=1)
