...
class MSResNet(nn.Module):
    def __init__(self, input_channel, layers=[1, 1, 1, 1], num_classes=12):
        ...
        # max-pooling kernel size: 16, 11, 6
    ...
    def _make_layer5(self, block, planes, blocks, stride=2):
        ...
        if stride != 1 or self.inplanes5 != planes * block.expansion:
            downsample = nn.Sequential(
                nn.Conv1d(self.inplanes5, planes * block.expansion, kernel_size=1, stride=stride, bias=False),
                nn.BatchNorm1d(planes * block.expansion),
            )
    ...
    def _make_layer7(self, block, planes, blocks, stride=2):
        ...
        if stride != 1 or self.inplanes7 != planes * block.expansion:
            downsample = nn.Sequential(
                nn.Conv1d(self.inplanes7, planes * block.expansion, kernel_size=1, stride=stride, bias=False),
                nn.BatchNorm1d(planes * block.expansion),
            )
...
def test_msr():
    ...
    x = torch.randn(9, 4, 32)
    msr_net = MSResNet(4)
    print(msr_net)

    y = msr_net(x)
    print(str(check_parameters(msr_net))+' Mb')
    print(y[1].shape)
