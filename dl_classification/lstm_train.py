...
import torch
import torchmetrics
from sklearn.metrics import accuracy_score, precision_score, recall_score, f1_score, matthews_corrcoef, confusion_matrix, average_precision_score, roc_auc_score
...
def parse_args():
    ...
    parser.add_argument('--epochs', type=int, default=49, help='upper epoch limit (default: 100)')
    ...
    parser.add_argument('--num_layers', type=int, default=4, help='# of stacked lstm layers (default: 4)')
    parser.add_argument('--hidden_size', type=int, default=60, help='number of hidden units per layer (default: 48)')
...
def train(net, train_loader, val_loader, optimizer, scheduler, criterion, num_epoch):
    ...
    for epoch in range(num_epoch):
        ...
        # save model
        save_model_path = os.path.join('./model_save/lstm/', 'model_{}_{}'.format(epoch, max_val_acc))
...
def validate(net, val_loader, criterion):
    ...
    with torch.no_grad():
        ...
        print('test_acc = %.4f, test_f1 = %.4f, test_p = %.4f, test_r = %.4f, test_mcc = %.4f.' % (acc, f1, precision, recall, mcc))
        # print(cm)
...
def main():
    ...
    net = LSTMClassifier(input_channels, num_classes, hidden_size, num_layers, dropout=dropout).to(device)
    # loss and optimizer
